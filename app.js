...
// ==========================================
// 2. DEBUG OVERLAY — mostra in tempo reale
// se MediaPipe vede pose/volto e quante ossa ci sono
// ==========================================
...
// Iniettato dinamicamente nella sidebar
// ==========================================
...
const injectMocapButton = () => {
    ...
    // Inserisce nella sidebar — cerca i selettori più comuni
    const sidebar =
    ...
    if (sidebar) {
        ...
        // Fallback: in alto a sinistra fisso
...
const setupRig = (loadedModel) => {
    ...
    model.traverse((obj) => {
        ...
        // FIX: legge le ossa da SkinnedMesh.skeleton.bones
        // obj.isBone non funziona su modelli GLB/FBX di Mixamo
        if (obj.isSkinnedMesh && obj.skeleton) {
            obj.skeleton.bones.forEach((bone) => {
                let name = bone.name.toLowerCase()
                    .replace(/mixamorig/gi, '')   // rimuove prefisso Mixamo
                    .replace(/:/g, '')             // rimuove ":"
                    .replace(/_/g, '')             // FIX: /g — rimuove TUTTI gli underscore
                    .replace(/\s/g, '')
                    .trim();
                skeleton[name] = bone;
                boneCount++;
...
const rigBone = (name, rotation, lerp = 0.3) => {
    ...
    try {
        ...
        console.error("Slerp error:", name, e);
    }
};

// ==========================================
// 9. CALLBACK MEDIAPIPE
// La camera gira sempre — le ossa si muovono
// SOLO quando mocapActive === true
// ==========================================
const videoElement = document.getElementById('input_video');

const onResults = (results) => {
    // Aggiorna sempre il debug (anche quando in pausa)
    const hasPose = results.poseLandmarks ? results.poseLandmarks.length : 0;
    const hasFace = results.faceLandmarks ? results.faceLandmarks.length : 0;
    dbg.update(hasPose, hasFace);

    // Guard: esce se mocap non è attivo o il modello non è caricato
